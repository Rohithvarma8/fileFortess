...
def des3_eax_encryption(file_bytes, key_hash):
    start_time = time.perf_counter()
    cipher = DES3.new(key_hash, DES3.MODE_EAX, nonce=b'0')
    new_file_bytes = cipher.encrypt(file_bytes)
    end_time = time.perf_counter()
    return (new_file_bytes, start_time, end_time)


def des3_eax_decryption(file_bytes, key_hash):
    start_time = time.perf_counter()
    cipher = DES3.new(key_hash, DES3.MODE_EAX, nonce=b'0')
...
if file_path:
    with open(file_path, 'rb') as input_file:
        ...
        if operation == '1':
            # Encrypt
            new_file_bytes, encryption_time_start, encryption_time_end = des3_eax_encryption(
                file_bytes, tdes_key)
            print("Time lapsed during the process:",
                  (encryption_time_end-encryption_time_start)*1000.0, "ms")
        else:
            # Decrypt
            new_file_bytes, decryption_start_time, decryption_end_time = des3_eax_decryption(
                file_bytes, tdes_key)
            print("Time lapsed during the process:",
